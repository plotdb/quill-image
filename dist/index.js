...
var Embed, Delta, fit2size, imagePlusBlot, resizer, ref$;
...
imagePlusBlot.prototype.format = function(n, v){
  if (n === 'width' || n === 'height') {
    ...
    return this.domNode.style.backgroundSize = fit2size(v);
  } else if (n === 'repeat') {
    return this.domNode.style.backgroundRepeat = v || 'no-repeat';
  } else {
...
resizer = function(){
  ...
  this._.dom.button.addEventListener('click', function(evt){
    var node, blot, quill, index, f, cur, cycle, i, next;
    evt.stopPropagation();
    if (!(node = this$._.editor.container.querySelector("[data-qip-key='" + this$._.key + "']"))) {
      return;
    }
...
ref$.create = function(opt){
  ...
  ref$.backgroundSize = fit2size(opt.fit);
  ref$.backgroundColor = 'rgba(0,0,0,.8)';
  ref$.backgroundPosition = 'center center';
  ref$.backgroundRepeat = opt.repeat || 'no-repeat';
...
ref$.formats = function(node){
  ...
  ret.fit = (v = s.backgroundSize) === "100% 100%" || !v ? "fill" : v;
  ret.repeat = s.backgroundRepeat || 'repeat';
  return ret;
};
